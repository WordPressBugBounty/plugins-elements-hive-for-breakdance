// original > https://github.com/robin-dela/hover-effect

/* modifications from original */
// replace tweenmax with gsap
// replace PlaneBufferGeometry with PlaneGeometry
// Add imageEl for canvas sizing based on the media element sizes
...
`
	...
	var imagesRatio = firstDefined(opts.imagesRatio, 1)
	...
	if (!mediaEl) {
		console.warn("Media Element missing")
		return
	}
	...
	renderer.domElement.style.width = parent.offsetWidth
	renderer.domElement.style.height = parent.offsetHeight
	...
	if (video) {
		...
		var texture1 = loader.load(image1, (t) => {
			onResize()
			render()
		})
	...
	const onResize = (e) => {
		if (mediaEl.offsetHeight / mediaEl.offsetWidth < imageAspect) {
			a1 = 1
			a2 = mediaEl.offsetHeight / mediaEl.offsetWidth / imageAspect
		} else {
			a1 = (mediaEl.offsetWidth / mediaEl.offsetHeight) * imageAspect
			a2 = 1
		}
		object.material.uniforms.res.value = new THREE.Vector4(mediaEl.offsetWidth, mediaEl.offsetHeight, a1, a2)
		renderer.setSize(mediaEl.offsetWidth, mediaEl.offsetHeight)
